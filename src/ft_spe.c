...
/*   Updated: 2016/09/27 22:03:24 by plefebvr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/ft_printf.h"

char		*ft_strsub_spe(t_info *info, int add, unsigned long long i)
{
	char	*ret;
...
static char	*part_one(t_info *info, char **str, int i)
{
	...
	{
		if (*str)
			ret = ft_strjoin_c_null(*str, info, info->lst->mfw_n);
	...
	{
		if (*str)
			ret = ft_strjoin_c_null(*str, info, 0);
		info->ret += (1);
		info->lst->ret = 1;
		ft_memdel((void **)&(*str));
		return (ret);
