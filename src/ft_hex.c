...
/*   Updated: 2016/09/28 19:13:27 by plefebvr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char			ft_get_hex(t_info *info, unsigned long long n)
{
	unsigned long long	i;
...
static char			*ft_itoa_hex(unsigned long long n, t_info *info)
{
	char		*str;
	long long	i;
...
static void			ft_hex_length(void *ap, t_info *info)
{
	if (ft_strcmp("hh", info->length) == 0)
		ft_put_in_lst(info, ft_itoa_hex((unsigned char)ap, info));
	if (ft_strcmp("h", info->length) == 0)
		ft_put_in_lst(info, ft_itoa_hex((unsigned short)ap, info));
	if (ft_strcmp("l", info->length) == 0)
		ft_put_in_lst(info, ft_itoa_hex((unsigned long)ap, info));
	if (ft_strcmp("ll", info->length) == 0)
		ft_put_in_lst(info, ft_itoa_hex((unsigned long long)ap, info));
	if (ft_strcmp("j", info->length) == 0)
		ft_put_in_lst(info, ft_itoa_hex((uintmax_t)ap, info));
	if (ft_strcmp("z", info->length) == 0)
		ft_put_in_lst(info, ft_itoa_hex((size_t)ap, info));
}
...
void				ft_hex(void *ap, t_info *info)
{
	...
	info->length ? ft_hex_length(ap, info) :
		ft_put_in_lst(info, ft_itoa_hex((unsigned)ap, info));
	pf_hex2(ap, info);
